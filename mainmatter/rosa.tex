...
Dies ist der Anteil an der Selbstinduktivit\"at, welcher vom magnetischen Feld
ausserhalb des Leiters verursacht wird. Zus\"atzlich muss jedoch auch das Feld
im Innern des Leiters ber\"ucksichtigt werden.
\todo[noline]{Herleitung im Artikel noch nicht ganz klar}

\schematic{selfStraightCylWireCrossSection}

Die   Feldst\"arke  am   Punkt   $Q   (x,0)$  im   Innern   des  Drahtes   ist
$\frac{2ix\mu}{\rho^2}$. Der auf  den Strom  normierte Fluss in  einem Element
des Drahtes mit L\"ange $l$ und der Breite $dx$ ist somit
...
\begin{equation}
    ...
    i \cdot L_2 
    = 
    l \cdot \int_{0}^{\rho} \frac{2ix}{\rho^2} \cdot \frac{x^2}{\rho^2}dx
    = 
    \frac{li}{2}
\end{equation}
...
Wenn  die   Permeabilit\"at  des   Drahtes  $\mu$  ist,   ist  der   Teil  der
Selbstinduktivit\"at, welcher vom internet Feld verursacht wird
...
\begin{align}
    ...
    \right]
    \quad \text{mit $L_2 = \frac{l}{2}$}
    \label{eq:sSCW:11}
    \\
    & \approx
    2l
    \left[
        \log{\frac{2l}{\rho}} - 1 + \frac{\mu}{4}
    \right]
    \quad \text{mit $L_2 = \frac{\mu l}{2}$}
    \label{eq:sSCW:111}
\end{align}
...
\begin{equation}
    L = 2 l \left[ \log{\frac{2l}{R}} - 1 + \frac{R}{l} \right]
    \label{eq:sRB:1}
\end{equation}
...
\begin{equation}
    ...
    L 
    = 
    2 l \left[ \log{\frac{2l}{\alpha + \beta}} 
    + \frac{1}{2} 
...
Unter  Benutzung  der  mittleren   geometrischen  Distanz  und  der  mittleren
arithmetischen Distanz kann noch eine genauere Formel f\"ur die Induktivit\"at
eines Streifens hergeleitet werden.  Die mathematischen Herleitungen f\"ur die
mittlere  geometrische und  die  mittlere arithmetische  Distanz  sind in  den
Anh\"angen  \ref{sec:gmd}  und  \ref{sec:amd} ab  Seite  \pageref{sec:gmd}  zu
finden.

Setzt  man die  dort  erhaltenen Werte  f\"ur $S_1^2$  und  $S_2^2$ in  Formel
\ref{eq:gmd:1} ein,  und ber\"ucksichtigt man,  dass $l$ viel kleiner  als $d$
ist, ergibt sich:
...
Streifens als Formel \ref{eq:tT:1}.
...
Die gegenseitige Induktivit\"at zweier paralleler, konzentrischer Kreise (ganz
rechts in Abbildung \ref{fig:circleStrip}) unter Vernachl\"assigung von Termen
vierter und h\"oherer Ordnung lautet:
...
Nun substituieren wir in Formel \ref{eq:sCS:2} gem\"ass:
...
Falls  der \"aussere  Leiter  eine nicht  vernachl\"assigbare Dicke  $a_3-a_2$
aufweist   (Abbildung  \ref{fig:concCond2}),   muss  $\log{a}$   in  Gleichung
\ref{eq:concCond:4} durch folgenden Ausdruck ersetzt werden:
\todo[noline]{%
    Hier  ist  der Artikel  ein  bisschen  unklar. Nirgends vorher  ist  $a_g$
    aufgetaucht. Ebenfalls  ist  $\log{a}$  nicht  als  solches  in  Gleichung
    \ref{eq:concCond:4}   vorhanden,  sondern   ein  Verh\"altnis. Nachrechnen
    erforderlich.%
}
...
% ---------------------------------------------------------------------------- %
\section{Nichtinduktive Windung um einen kreisf\"ormigen Zylinder}
